...
{
    internal sealed class CachePreventingRequestHandler : IRequestHandler
    {
        ...
        {
            if (request.Url.Contains("DoNotCache"))
            {
                request.Headers = SetCacheBlockingHeaders(request.Headers);
            }
        ...
        private NameValueCollection SetCacheBlockingHeaders(NameValueCollection headers)
        {
